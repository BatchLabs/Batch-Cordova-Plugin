...
- (nullable BACSimplePromise<NSString*> *)doAction:(NSString *)action withParameters:(NSDictionary *)parameters {
    ...
    else if ([action caseInsensitiveCompare:INBOX_DISPLAY_LANDING_MESSAGE] == NSOrderedSame)
    {
        resultPromise = [self displayLandingMessage:parameters];
    }
...
- (NSDictionary*)serializeNotificationContent:(BatchInboxNotificationContent*)content
{
    ...
    json[@"source"] = @(source);
    json[@"hasLandingMessage"] = @(content.hasLandingMessage);
...
- (BACSimplePromise<NSString*>*)displayLandingMessage:(NSDictionary*)parameters {
    NSError *error = nil;
    BatchInboxFetcher *fetcher = [self fetcherInstanceForParameters:parameters error:&error];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (associatedNativeNotification != nil) {
            [associatedNativeNotification displayLandingMessage];
        } else {
            NSLog(@"[Batch] Inbox: Could not display notification landing message: No matching native notification. This can happen if you kept a JavaScript instance of a notification but are trying to use it with another fetcher, or if the fetcher has been reset inbetween.");
        }
